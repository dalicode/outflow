...
import { DASHBOARD_VIEWS, MONTH_SPANS } from '../features/dashboard/constants'

const MONTH_REGEX = /^\d{4}-(0[1-9]|1[0-2])$/

export function parseMonthParam(value: string | null, defaultKey: string): string {
  if (!value || !MONTH_REGEX.test(value)) return defaultKey
  return value
}

export function parseYearParam(value: string | null, defaultYear: number): number {
  const year = parseInt(value ?? '', 10)
  if (Number.isNaN(year) || year < 2000 || year > 2100) return defaultYear
  return year
}

export function parseViewParam(value: string | null): DashboardView {
...
export function parseAnalyticsMonthParam(value: string | null, maxMonth: number): number | null {
  const month = parseInt(value ?? '', 10)
  if (Number.isNaN(month) || month < 0 || month > maxMonth) return null
  return month
}
