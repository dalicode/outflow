...
export default function Navbar({ onAddExpense, syncDot, onSignOut, userEmail }) {
  ...
  const inactiveClass = 'text-theme-muted hover:bg-theme-background hover:text-theme-text'

  return (
    <nav className="navbar-theme sticky top-0 z-30">
      <div className="max-w-4xl mx-auto px-4 flex items-center justify-between h-14">
        <span className="text-theme-text font-semibold text-lg tracking-tight">Spending Tracker</span>
        ...
        <div className="flex sm:hidden items-center gap-2">
          ...
          <button onClick={onAddExpense}
            ...
            aria-label="Add expense">+</button>
          <button onClick={() => setOpen((o) => !o)} aria-label="Toggle menu" className="text-theme-text p-1">
            {open ? '✕' : '☰'}
          </button>
        </div>
      </div>
      ...
      {open && (
        <div className="sm:hidden bg-theme-surface/95 backdrop-blur-md border-b border-theme-border px-4 pb-3 flex flex-col gap-0.5">
          ...
          {onSignOut && (
            <button onClick={onSignOut} className="text-left px-3 py-2 text-theme-muted hover:text-theme-danger text-sm font-medium">
