...
export function resolveMoneyLocaleConfig(
...
export function centsToSignedDollars(
  cents: number,
  isNegative: boolean,
): number {
  if (cents === 0) return 0;
...
export function clampMoneyCents(
  ...
  const maxCents =
    options?.maxCents ?? (options?.maxAmount != null ? dollarsToCents(options.maxAmount) : undefined);
...
export function formatCurrencyFromCents(
  ...
  return new Intl.NumberFormat(options.locale, {
    ...
    options.isNegative && cents !== 0 ? -(Math.abs(cents) / 100) : cents / 100,
  );
}
...
export function parsePastedMoneyInput(
  ...
  const isNegativePaste =
    ...
    (/^-/.test(trimmedText) || /^\(.*\)$/.test(trimmedText));
