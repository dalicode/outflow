...
export function computeMultiMonthCategoryRows(
  ...
  monthKeys.forEach((_, idx) => {
    Object.keys(monthlyTotals[idx]).forEach((name) => {
      allNames.add(name)
    })
  })
  ...
  const rows: MultiMonthCategoryRow[] = Array.from(allNames).map((name) => {
    const monthlyAmounts = monthKeys.map((_, idx) => monthlyTotals[idx][name] || 0)
 ...
 */
export function computeMultiMonthFixedRows(monthSummaries: MonthlySummary[]): MultiMonthFixedRow[] {
  ...
  return Array.from(idToName.entries())
    .map(([id, name]) => ({
      id,
      name,
      monthlyAmounts: monthSummaries.map((_, idx) => monthLookup[idx][id] ?? null),
    }))
