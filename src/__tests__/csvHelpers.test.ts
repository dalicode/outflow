...
describe('parseCSV', () => {
  it('parses quoted commas in payee and description', () => {
    const csv = [
      '"Date","Category","Payee","Description","Amount","Month","Year"',
    ...
    expect(rows[0].description).toBe('Lunch, team')
  })
  ...
  it('supports CRLF line endings', () => {
    ...
    expect(rows[0].description).toBe('April rent')
  })
  ...
  it('round trips an outflow export-style row from expenseToRow', () => {
    const expense: Expense = {
      ...
      categoryId: 1,
      payeeId: 2,
      description: 'Coffee with "Sam", downtown',
    }
    ...
    expect(rows[0].description).toBe('Coffee with "Sam", downtown')
