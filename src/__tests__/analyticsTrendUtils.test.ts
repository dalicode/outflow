...
describe("buildCategoryBreakdown", () => {
  ...
  it("sorts by amount descending", () => {
    ...
    const variableRows = [
      { key: "a", name: "Small", amounts: amounts.map((_, i) => i === 4 ? 50 : 0), yearTotal: 50 },
      { key: "b", name: "Large", amounts: amounts.map((_, i) => i === 4 ? 300 : 0), yearTotal: 300 },
      { key: "c", name: "Medium", amounts: amounts.map((_, i) => i === 4 ? 150 : 0), yearTotal: 150 },
    ];
    const data = makeAnalyticsData({ variableRows });
    const rows = buildCategoryBreakdown(data, 4);
  ...
  it("sets all pct to 0 when total is 0", () => {
    const variableRows = [
      { key: "a", name: "A", amounts: Array(12).fill(0), yearTotal: 0 },
    ];
    const data = makeAnalyticsData({ variableRows });
    const rows = buildCategoryBreakdown(data, 4);
...
describe("buildPayeeBreakdown", () => {
  ...
  it("sorts by amount descending", () => {
    const amounts = Array(12).fill(0);
    const payeeRows = [
      { key: "p1", name: "Small", amounts: amounts.map((_, i) => i === 4 ? 50 : 0), yearTotal: 50 },
      { key: "p2", name: "Large", amounts: amounts.map((_, i) => i === 4 ? 400 : 0), yearTotal: 400 },
    ];
    const data = makeAnalyticsData({ payeeRows });
    const rows = buildPayeeBreakdown(data, 4);
    expect(rows[0].name).toBe("Large");
...
describe("buildMonthDrilldownData", () => {
  ...
  it("populates all sub-arrays", () => {
    const amounts = Array(12).fill(0);
    const variableRows = [
      { key: "a", name: "Groceries", amounts: amounts.map((_, i) => i === 4 ? 300 : 0), yearTotal: 300 },
    ];
    const expenses: Expense[] = [
