...
interface PullToRefreshContainerProps extends Omit<
...
const PullToRefreshContainer = forwardRef<
  HTMLDivElement,
...
>(function PullToRefreshContainer(
  ...
  const handleScroll = useCallback(
    (event: React.UIEvent<HTMLDivElement>) => {
      onScroll?.(event);
      if (
        (scrollTargetRef?.current?.scrollTop ?? event.currentTarget.scrollTop) >
          0 &&
        !isRefreshing &&
        pullDistance > 0
      ) {
  ...
  const instructionOpacity =
    isRefreshing || showRefreshSuccess
      ? 0
      : instructionReveal * (1 - spinnerReveal);
  const refreshAreaHeight =
    indicatorVisible || showRefreshSuccess || isRefreshing
      ? Math.min(
          ...
          Math.max(
            44,
            pullDistance * 0.6 + (isRefreshing || showRefreshSuccess ? 18 : 10),
          ),
        )
      : 0;
  const contentStyle =
