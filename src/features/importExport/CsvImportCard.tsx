...
import ImportReviewModal, { type ImportReviewSelection } from './ImportReviewModal'
...
export default function CsvImportCard({
  ...
  const finalizeImport = async (
    ...
    const selectionMap = new Map(selectedOverrides.map((selection) => [selection.rowId, selection]))
    ...
    const importedYears = [...new Set(toAdd.map((row) => parseInt(row.date.slice(0, 4), 10)))].sort(
      (a, b) => a - b,
    )
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>, nextReplaceMode: boolean) => {
    e.stopPropagation()
  ...
  return (
    <Card title="Import CSV" className="flex-1" variant={variant}>
      <p className="text-xs text-theme-muted mb-2">
        Import expenses from a CSV file. We’ll review uncertain payee matches before anything is
        saved.
      </p>
