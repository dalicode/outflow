...
test.describe("Edit Historical Data", () => {
  test("first-time backfill save writes snapshots", async ({ page }) => {
    ...
    await expect
      .poll(async () => (await getIncomeSnapshots(page)).filter((snapshot) => snapshot.year === targetYear).length)
      .toBe(12);
    await expect
      .poll(async () => (await getSavingsSnapshots(page)).filter((snapshot) => snapshot.year === targetYear).length)
      .toBe(12);
    await expect
      .poll(async () => (await getFixedExpenseSnapshots(page)).filter((snapshot) => snapshot.year === targetYear).length)
      .toBe(12);

    const incomeSnapshots = (await getIncomeSnapshots(page)).filter((snapshot) => snapshot.year === targetYear);
    const savingsSnapshots = (await getSavingsSnapshots(page)).filter((snapshot) => snapshot.year === targetYear);
    const fixedSnapshots = (await getFixedExpenseSnapshots(page)).filter((snapshot) => snapshot.year === targetYear);
  ...
  test("editing existing historical snapshots and resaving updates persisted values", async ({ page }) => {
    const now = new Date();
    const targetYear = now.getFullYear() - 1;
  ...
  test("current-year tab only allows past months and persists only editable month range", async ({ page }) => {
    const now = new Date();
    ...
    if (editableMonthCount === 0) {
      ...
      await expect(dialog.getByText("No historical data available. There are no past months to edit yet.")).toBeVisible();
      await dialog.getByRole("button", { name: "Cancel" }).click();
      await expect(dialog).not.toBeVisible();
      expect((await getIncomeSnapshots(page)).filter((snapshot) => snapshot.year === currentYear)).toHaveLength(0);
      return;
    }
    ...
    await expect(endMonthSelect.locator("option").last()).toHaveText(MONTH_NAMES[editableMonthCount - 1]);
    if (editableMonthCount < 12) {
      await expect(endMonthSelect.locator("option", { hasText: MONTH_NAMES[editableMonthCount] })).toHaveCount(0);
    }

    await saveHistoricalModal(dialog);

    const incomeSnapshots = (await getIncomeSnapshots(page)).filter((snapshot) => snapshot.year === currentYear);
    expect(incomeSnapshots).toHaveLength(editableMonthCount);
    expect(incomeSnapshots.every((snapshot) => snapshot.month >= 1 && snapshot.month <= editableMonthCount)).toBe(true);
  });

  test("draft restore and discard uses draft banner flow without persisting unsaved edits", async ({ page }) => {
    const now = new Date();
    const targetYear = now.getFullYear() - 1;
    ...
    expect((await getIncomeSnapshots(page)).filter((snapshot) => snapshot.year === targetYear)).toHaveLength(0);
    ...
    await expect(dialog.getByText("Unsaved changes restored from your last session.")).toHaveCount(0);
