...
async function setContextOffline(
...
async function waitForZeroPendingSync(page: import('@playwright/test').Page): Promise<void> {
  await waitForSyncSettled(page)
}
...
test.describe('Live Supabase sync (UAT)', () => {
  ...
  test('offline edit syncs after reconnect', async ({ browser, liveUser }) => {
    ...
    try {
      ...
      await expect
        .poll(
          async () =>
            (await getAllExpenses(pageA)).find((x) => x.localId === expenseOnA?.localId)?.amount ??
            0,
        )
        .toBe(88.45)
      await expect
  ...
  test('concurrent edit conflict resolves to a single stable value', async ({
    browser,
    liveUser,
  }) => {
    const contextA = await browser.newContext()
    ...
    try {
      ...
      await updateExpenseForSyncTestWithTimestamp(
        pageA,
        shared?.id as number,
        { amount: 45.01 },
        earlierUpdateAt,
      ...
      await expect
        .poll(
          async () =>
            (await getAllExpenses(pageA)).find((x) => x.localId === shared?.localId)?.amount ?? 0,
        )
        .toBe(49.99)
      await expect
        .poll(
          async () =>
            (await getAllExpenses(pageB)).find((x) => x.localId === shared?.localId)?.amount ?? 0,
        )
        .toBe(49.99)

      const amountA = (await getAllExpenses(pageA)).find(
        (x) => x.localId === shared?.localId,
      )?.amount
      const amountB = (await getAllExpenses(pageB)).find(
        (x) => x.localId === shared?.localId,
      )?.amount
  ...
  test('delete undo syncs across contexts after a remote delete', async ({ browser, liveUser }) => {
    ...
    try {
      ...
      await expect
        .poll(async () =>
          (await getAllExpenses(pageA)).some((x) => x.localId === createdOnA?.localId),
        )
        .toBe(false)
  ...
  test('interrupted sync resumes safely without duplicates', async ({ browser, liveUser }) => {
    ...
    try {
      ...
      await expect.poll(async () => getSyncMetadataCounts(pageA)).toEqual({ pending: 1, failed: 0 })
      await expect
        .poll(async () => {
  ...
  test('pending changes persist until reconnect and sync status reflects transitions', async ({
    browser,
    liveUser,
  }) => {
  ...
  test('user can delete all data and return to an empty sync state', async ({
    browser,
    liveUser,
  }) => {
    const contextA = await browser.newContext()
